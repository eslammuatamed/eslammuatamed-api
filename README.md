...
# لا تُنشئ قاعدة اختبار ثابتة: حزمة e2e تُنشئ قاعدتها وتُسقطها بنفسها — انظر قسم e2e أدناه.
...
في كل تشغيل تُولّد اسمًا فريدًا `eslammuatamed_e2e_<run-id>`، تُنشئه وتُرحّله وتبذره،
...
**لا أوسمة (`tags`).** دمج PR لترقية `dev → main` أو لإصلاح عاجل يولّد حدث `push: main`، فيُشغّل `deploy.yml` على ذلك الـ `SHA` بعينه؛ أمّا الدفع المباشر فيرفضه الـ ruleset. وقد يُشغَّل **تشغيلان** لنفس الـ `SHA`: حدث `push: main`، ويُضاف إليه احتياطيّ `deploy-fallback.yml` عند دمج الـ PR (لأنّ حدث الدفع يسقط أحيانًا) فيُرسِل نفس الخطّ بـ `workflow_dispatch`. مجموعة التزامن الواحدة و`preflight` تجعلان الزوج مُتماثلًا (idempotent): **تشغيل واحد على الأكثر يبلغ الكتابة على الخادم**.
...
`UNLICENSED` — لا تُمنَح رخصة استعمال أو إعادة توزيع. المستودع **عموميّ للاطّلاع** منذ `2026-08-12`؛ العموميّة رؤية، لا ترخيص.
