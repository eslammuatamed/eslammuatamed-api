...
**A failed deploy is not automatically a safe state.** Rollback needs somewhere to roll back *to*. If there is no previous release, or the rollback target also fails verification, the run goes red announcing `MANUAL INTERVENTION REQUIRED` — and nothing is reverted: the unhealthy release is still `current`, whether it is serving badly or not serving at all. Treat that outcome as an **ongoing outage**, not a safe abort.
