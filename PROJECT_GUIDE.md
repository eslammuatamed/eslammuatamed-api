...
> خارج النموذج القانوني. الترتيب أدناه يحترم الاعتماديّات **ثمّ** يتدرّج في كثافة المفاهيم.
...
4. [`locales`](src/modules/locales/README.md) — **٧٦ سطرًا، وتعتمد عليها عشر وحدات.** أصغر وحدة حقيقية وأكثر مقطع مشترك في المستودع؛ ستراها في كل `service` بعدها.
...
8. [`redirects`](src/modules/redirects/README.md) — **١٤٧ سطرًا وأدقّ ممّا يوحي حجمها:** `buildRedirectOps` **لا تُنفِّذ** شيئًا، بل تُعيد ثلاث عمليّات يدفعها المُستدعي في `$transaction` الخاصّ به. أوّل درس في «مَن يملك حدود المعاملة».
...
10. **الحافة الصعبة، أخيرًا وبهذا الترتيب:** [`media`](src/modules/media/README.md) → [`auth`](src/modules/auth/README.md) → [`access-control`](src/modules/access-control/README.md) → [`contact`](src/modules/contact/README.md) → [`preview`](src/modules/preview/README.md). خمس وحدات تحمل أكثف المفاهيم: خطّ معالجة وسائط بحدّ تزامن، تجزئة `argon2id` وتدوير `refresh`، صلاحيات تُحلّ من قاعدة البيانات مع ترتيب حرّاس عالميّ، `idempotency` وإرسال بعد الالتزام، وأخيرًا توقيع `HMAC` بنافذة صلاحية ومقارنة ثابتة الزمن. `preview` آخرًا لأنّها الأعمق في الرسم (عمق **3**).
