...
أصلًا: يخرج السكربت بالفشل معلنًا `MANUAL INTERVENTION REQUIRED`، **ويبقى الإصدار المعطوب هو
`current` وتبقى الخدمة تعمل عليه** — وكذلك حين يفشل هدف التراجع نفسه في إعادة التحقّق. فالنشر
الأحمر هنا **انقطاع قائم** يستدعي تدخّلًا فوريًّا، لا حالة أمِنة. الهجرات **تُصحَّح بالتقدّم لا
