...
8. [`redirects`](src/modules/redirects/README.md) — **١٤٧ سطر شيفرة (بالمقياس نفسه؛ `wc -l` الخام يعطي ١٩٥)، وأدقّ ممّا يوحي حجمها. وهنا أوّل لقاء لك بـ `$transaction` في `Prisma`:** `buildRedirectOps` **لا تُنفِّذ** شيئًا، بل تُعيد ثلاث عمليّات يدفعها المُستدعي في `$transaction` الخاصّ به. أوّل درس في «مَن يملك حدود المعاملة».
