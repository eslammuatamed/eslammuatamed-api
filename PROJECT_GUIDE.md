...
> **قياس أم توقُّع؟** الزمن حتى ظهور طلب الموافقة كان **`203` ثانية مقيسة** على التشغيل
> `31837891096`. الرقم اللاحق **`≈131` ثانية متوقَّع** (`projected`) لا مقيس، لأنّ `deploy.yml`
> لا يعمل إلّا على `push: main`. لا يُعامَل بوصفه محقَّقًا قبل أن تقيسه نشرة `main` حقيقيّة.
