...
تعني «سليم فعلًا»، لا «يستمع على المنفذ». وإن لم يوجد إصدار سابق يُتراجَع إليه فلا تراجع
أصلًا: يخرج السكربت بالفشل معلنًا `MANUAL INTERVENTION REQUIRED`، **ولا يُرَدّ شيء: يبقى الإصدار المعطوب هو
`current`** — يخدم خدمةً معطوبة أو لا يخدم أصلًا — وكذلك حين يفشل التحقّق من هدف التراجع نفسه.
فالنشر الأحمر هنا **انقطاع قائم** يستدعي تدخّلًا فوريًّا، لا حالة آمنة. الهجرات **تُصحَّح بالتقدّم لا
