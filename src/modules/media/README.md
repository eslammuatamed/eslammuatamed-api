...
حدود الرفع نفسها ترفض قبل أي معالجة: طلبٌ بلا جزء ملفّ باسم `file`، أو بملفّ تحت اسم حقلٍ غير متوقَّع، يجيب **`400`** (استجابة `ProblemDetails` RFC 7807) دون استدعاء `MediaService` ودون أثرٍ على التخزين أو قاعدة البيانات.
...
الكائنات تُكتَب **قبل** صفّ `DB`؛ عند أي فشل **قبل حفظ الصفّ** بعد الكتابة يُستدعى `compensate`: `storage.deleteMany` للمفاتيح التي سجّلها هذا الطلب — أي ما نجح `put`ه **من منظور هذه العملية**؛ وكتابةٌ نجحت على `R2` وضاعت استجابتها **قد** تُرفَض محلّيًّا (الـ `SDK` يجري **ثلاث محاولات** كحدّ أقصى افتراضيًّا — أي محاولة أصليّة وإعادتان — فضياع استجابة واحدة لا يُفشِل النداء بالضرورة)؛ ومتى رُفِضت لم يُسجَّل مفتاحها ولم يعرفه التعويض (وهو نظير التزامِ `PostgreSQL` غير المحسوم أدناه، في جانب التخزين). سباق محتوى مكرِّر يُحلّ لصالح **الفائز** (إعادة فحص `contentHash`) — **إذا أعادت القراءة صفَّ الفائز، سواء نجح التنظيف أو سُجِّل فشله**؛ وإلّا خرج الخطأ. **والضمانان ليسا واحدًا، فلا تقرأهما ضمانًا واحدًا.**
...
**حدود التعويض في التنفيذ الحالي:** يقتصر تعويض الرفع على ما قبل تأكّد `mediaAsset.create`
من حفظ صفّ الوسيط. فإذا نجحت كتابة الكائنات ثم فشلت محاولة الحفظ، يحاول `deleteMany` حذف المفاتيح
التي نجح هذا الطلب في تسجيلها، لكن التنظيف **أفضل جهد** وقد تبقى كائنات يتيمة عند فشل خارجي أو شبكي؛
ولا توجد حاليًا مصالحة آلية لهذه الكائنات. أمّا بعد نجاح `mediaAsset.create`، فالتسجيل أو تحويل
الاستجابة اللاحق لا يشغّل تعويض التخزين، حتى لا يُحذف كائن يشير إليه صفّ محفوظ. وإذا رفض التنظيف
نفسه، يُسجَّل الفشل عبر `media.compensation_incomplete` ولا يستبدل رسالة خطأ الرفع/الحفظ الأصلية
في مسار الفشل العادي؛ أمّا سباق التكرار فيستمرّ في إعادة قراءة الأصل الفائز بدل تحويل فشل التنظيف
إلى خطأ جديد. لا يحلّ هذا الترتيب غموض انقطاع الشبكة حول التزام قاعدة البيانات؛ لذلك لا يُعدّ ضمانًا
لعدم وجود الكائنات اليتيمة.
