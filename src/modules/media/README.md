...
الصفّ آخر ما يُكتَب في المسارين (`storage.put` للكائنات، ثمّ `mediaAsset.create`)، فأيّ فشل **في عمل
الرفع نفسه** يقع قبل وجود الصفّ ولا يخلّف صفًّا يتيمًا. وحذف الكائنات **بذل أفضل** لا أكثر: المحوّل
...
2. **حدّ الـ `catch` يمتدّ إلى ما بعد `create`.** التسجيل وتحويل الكيان يقعان داخل الـ `try` نفسه،
   فاستثناء فيهما يُشغّل التعويض **والصفّ مُلتزَم بالفعل** — وهو المسار الوحيد الذي يُنتِج صفًّا يتيمًا.
