...
**والترتيب أعلاه ليس تفصيلًا تجميليًّا.** ما قبل إزالة التكرار هو تحقّق هويّة رخيص لا يستدعي
`Sharp`؛ وما بعدها — سقف البكسلات والمعالجة كلّها — داخل الـ limiter. ينتج عن ذلك سلوكٌ لا يُخمَّن:
**إعادة رفع صورة مطابقة بالبايتات تتجاوز `40 MP` تعود `200` مُزالة التكرار، لا `422`،** لأنّ السقف
لم يُبلَغ أصلًا. أمّا تزييف الامتداد/النوع فيُرفض بـ `422` **حتى للمكرَّر**، لأنّ تحقّق الهويّة يسبق
البحث عمدًا.
