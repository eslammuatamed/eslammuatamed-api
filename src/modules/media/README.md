...
يدويًّا. كائن يتيم = بايتات لا يشير إليها أيّ ردّ بعد الآن — **لا «لا يراها أحد»:** روابط الكائنات عامّة ومُخزَّنة مؤقّتًا سنةً كاملة، فرابطٌ سُلِّم قبل الحذف قد يظلّ يعمل. وهو مع ذلك **أأمن** من صفّ حيّ يشير إلى كائن غير موجود —
...
الكائنات تُكتَب **قبل** صفّ `DB`؛ عند أي فشل بعد الكتابة يُستدعى `compensate`: `storage.deleteMany` للمفاتيح التي سجّلها هذا الطلب — أي ما نجح `put`ه **من منظور هذه العملية**؛ وكتابةٌ نجحت على `R2` وضاعت استجابتها تُرفَض محلّيًّا فلا يُسجَّل مفتاحها ولا يعرفه التعويض (وهو نظير التزامِ `PostgreSQL` غير المحسوم أدناه، في جانب التخزين). سباق محتوى مكرِّر يُحلّ لصالح **الفائز** (إعادة فحص `contentHash`). **والضمانان ليسا واحدًا، فلا تقرأهما ضمانًا واحدًا.**
...
الكائن اليتيم بايتات مهدورة لا يراها أحد؛ الصفّ اليتيم عطلٌ يراه المستخدم — والفرق بينهما هو سبب
