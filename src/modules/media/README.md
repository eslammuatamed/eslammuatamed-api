...
- **سقف المعالجة:** `MAX_CONCURRENT_PROCESSING = 2` متزامنة لكل نسخة؛ رفعٌ إضافي **يحتاج معالجة** أثناء الانشغال → **`429`** + `Retry-After` (`PROCESSING_RETRY_AFTER_SECONDS = 2`) ولا يُصفّ أبدًا. أمّا إصابة إزالة التكرار فتعود **قبل** سقف التزامن ولا تستهلك خانة. **وانتبه إلى تصادم اللفظ:** «السقف» هنا هو تزامن المعالجة (`2`)، وليس سقف البكسلات (`40 MP`) — حدّان لا علاقة بينهما، وإن كان المكرَّر يعود قبل كليهما.
