...
- **لا `e2e` لرفع `PDF`:** رفع سيرة `PDF` **من عمل هذه الوحدة** (انظر صدر الملفّ)، لكنّه خارج نطاق **اختبارات `e2e`** هنا؛ تغطية `resumeAsset = null` + التحقّق البنيوي لواصف `PDF` كافيان.
