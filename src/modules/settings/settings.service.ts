...
export class SettingsService {
  ...
  async getPublicSettings(locale: string): Promise<PublicSiteSettingsEntity> {
    ...
    return {
      ...
      // asset id. `updateSettings` below rejects a non-PDF asset on write; the kind check here is
      // defence in depth.
      resumeAsset:
  ...
  async updateSettings(
    ...
    // The resume slot may only reference a PDF asset — this service owns that rule, and the
    // check immediately below is where it is enforced. null
