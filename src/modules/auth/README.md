...
**حالة التوافق:**
- **حارس قائم على `JwtService` بدل `passport-jwt`:** `Compatible`. توثيق `NestJS → Security → Authentication` الحالي يعلّم بالضبط هذا النمط (`JwtModule.registerAsync`، `signAsync`، `verifyAsync`، حارس يقرأ الـ Bearer). وهو أيضًا **انحراف مقصود موثّق** عن عادة `passport-jwt`: `principle 16` يذكر هذا المثال حرفيًّا كنمط تجاوزه التوثيق، ويقرّه `D00-6`. (لا وجود لأي تبعية `passport*` في هذا الأساس.)
