...
- **الكتالوج كود، المنح بيانات.** كل مفتاح صلاحية (`articles.read`, `articles.update`, …) مُعرَّف في `permissions.ts` ومرتبط بدالّة محروسة، فلا يمكن إنشاؤه عبر الـ API (يُكشف للقراءة فقط عبر `GET /admin/permissions`). والكتالوج يسرد القدرات المُنفَّذة فعليًا فقط: اختبار `route-permissions.spec.ts` يُثبت الاتجاهين معًا، فأي مفتاح لا يحرس مسارًا يُسقط البناء (D09-7). المشغّل ينشئ الأدوار ويمنحها مفاتيح من هذا الكتالوج (صفوف `RolePermission`).
...
- **المنحة الوحيدة القابلة للتخزين إمّا مفتاح كتالوج أو `'*'`.** الإنفاذ يقع في الـ DTO لا في الخدمة: `@IsIn(GRANTABLE_PERMISSIONS, { each: true })` على الحقل `permissions` في `CreateRoleDto` و`UpdateRoleDto` (`dto/role.dto.ts`). فمفتاح مجهول يُرفض بـ 422 عند `ValidationPipe` قبل أن يصل إلى الخدمة أو قاعدة البيانات.
  ...
  >
  > *وهنا يجب فصل شيئين خلطهما يُنتج إحصاءً لا ينتهي.* **قيود الطول عند الحدّ وحده هي القاعدة
  > العامّة هنا، لا الاستثناء:** لا عمود واحد يحمل قيد طول (`@db.VarChar`/`@db.Char` — صفر)، لأنّ
  > `String` في `Prisma` يصير `TEXT` بلا حدّ. فكلّ `@MaxLength` في الـ `DTOs` يُنفَّذ عند الحدّ ولا
  > شيء خلفه. **لا تعدَّ هذه واحدةً واحدة؛ اعرف القاعدة: مقاسٌ ⇒ الحدُّ وحده.**
  >
  > > ⚠ **أمّا الشكل فلا تعمّم فيه، ولا تسأل `schema.prisma` عنه.** `Prisma` **لا يستطيع تمثيل قيود
  > > `CHECK`**، فهي تُكتب في `SQL` الهجرات ولا تظهر في المخطّط إطلاقًا — وهذا مكتوب بنصّه في
  > > `prisma/migrations/20260805110000_add_skill_slug/migration.sql`. وثمّة قيود `CHECK` فعليّة تسند
  > > الشكل: `skills_slug_format_check` يفرض `kebab-case` ويمنع الشكل `uuid` عند العمود،
  > > و`media_assets_content_hash_sha256_hex_check` يفرض شكل الـ `hash`. **فقاعدةُ «الشكل عند الحدّ
  > > وحده» خاطئة، ومَن يتحقّق منها بمسح `schema.prisma` يحصل على تأكيد كاذب: أداةُ المسح عمياء عن
  > > الدليل بحكم التصميم.** اسأل `prisma/migrations/**.sql`.
  >
