...
- **Controller رفيع، Service سمين — قاعدةُ المستودع لا قانونَه:** الـ controller يوجّه ويربط الـ DTO ويحمل decorators الـ Swagger، والمنطق في الـ service (`principle 13`). **والاستثناء الوحيد المقصود في `preview`:** الـ controller هناك يستدعي `verify()` بنفسه ويحوّل الفشل إلى `404`، لأنّ إخفاء وجود المسودّة **هو** القرار الأمنيّ نفسه. إن وجدت منطقًا في `controller` غيره فهو عيب لا نمط.
...
و`422` وحده في هذا الجدول له معنى واحد ثابت: **قُرئ الجسم وفُهم وفشل التحقّق عليه** (أو رفضته
قاعدة فرادة). **أمّا `400` فلا يدلّ على طبقة بعينها**، والجدول أعلاه يُثبت ذلك بثلاثة مواضع
متباعدة: `ParseUUIDPipe` على شكل المسار **قبل** الجسم، و`assertEnabled` **داخل الـ service** بعد أن دخل الطلب
إليها (وأكثر مواضعه قراءات `GET` عامّة بلا جسم أصلًا)، والفرع الافتراضي في `AllExceptionsFilter` **في نهاية المسار كلّه**. *لا تقرأ
`400` بوصفه اسمًا لطبقة؛ اقرأه بوصفه «رُفِض، والطبقة التي رفضته تُعرَف من الشيفرة لا من الرمز».*
