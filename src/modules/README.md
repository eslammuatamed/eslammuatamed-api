...
- **Controller رفيع، Service سمين:** الـ controller يوجّه ويربط الـ DTO ويحمل decorators الـ Swagger، و**قواعد المجال** كلّها في الـ service (`principle 13`).
...
مُسنَدًا يُرجِع `422` بلا جسم (`access-control`)، وكذلك رفض نوع الملفّ أو حجمه أو بنيته في `media`. **أمّا `400` فلا يدلّ على طبقة بعينها**، وله في هذا المستودع **خمسة مصادر متباعدة**:
