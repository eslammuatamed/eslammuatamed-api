...
### قائمة الإدارة — `GET /admin/articles?page=&perPage=&status=&q=`
القراءة الإدارية محروسة بـ `articles.read` وتعيد خريطة الترجمات الكاملة. `status`
اختياري، و`q` اختياري بطول أقصاه 120: بعد `trim`، القيمة الفارغة لا تضيف مرشّحًا؛
والقيمة غير الفارغة تطابق **`ArticleTranslation.title` فقط** بمطابقة جزئية غير
حسّاسة لحالة الأحرف عبر **كل** اللغات المؤلّفة. لا يعيد هذا استعمال FTS العام، فلا
يبحث في `excerpt` أو `body` أو `slug` أو التصنيفات. يلتقي `q` مع `status` بـ AND،
ويستخدم صفّ المقالات والعدّ نفس الشرط. الترتيب `createdAt desc` ثم `id asc` حتى
تبقى حدود الصفحات ثابتة عند تساوي زمن الإنشاء.
...
`articles.service.spec.ts` (القوائم/التفاصيل/الكتابة/الترقية)، و e2e: `test/articles.e2e-spec.ts` + `test/articles-related.e2e-spec.ts` (تُثبت إخفاء المسودّات، بحث العنوان الإداري ثنائي اللغة، ترتيب المرتبطة، مطابقة العقد).
