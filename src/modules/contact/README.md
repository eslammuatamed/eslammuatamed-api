...
(`SENT` / `FAILED` / `PENDING`). ما تصفه هذه الوحدة هو طرف الـ backend وحده؛ وجود واجهة تستهلكه
في `/dashboard` سؤال يخصّ مستودع الويب، وحالة النشر يملكها سجلّ الإصدارات.
...
| `idempotency-key.pipe.ts` | التحقّق من ترويسة `Idempotency-Key` كـ pipe، أي **قبل** أيّ قراءة من قاعدة البيانات. وهو **موضع الإنفاذ الوحيد لشكل المفتاح**: الخدمة تخزّنه كما جاء، والعمود `String` حرّ — والفرادة لكلّ رسالة مسنودة في `DB`، أمّا الطول والمحارف فلا. وهذا هو **الوضع الطبيعيّ لقيود المقاس** في هذا المستودع لا استثناءً فيه. أمّا **الشكل** فمختلط: شكل هذا المفتاح بلا سند، بينما شكلا `slug` المهارة و`hash` الوسائط يسندهما قيدا `CHECK` في `SQL` الهجرات (وهما وحدهما قيدا **الشكل**؛ وثمّة قيود `CHECK` أخرى للمدى والاتّساق — والمرجع `prisma/migrations/**.sql` لا `schema.prisma`) ([التفصيل](../access-control/README.md)) |
...
## الاحتفاظ والتطهير (`D09-14` / `D19-10`)
...
- **لماذا عمود مخصَّص لا `updatedAt`؟** `@updatedAt` تعني «آخر تعديل» (تبديل `isRead` أو إعادة الأرشفة يحرّكها)،
  فتُعيد تعريف نافذة الاحتفاظ ضمنًا وقد تحذف بيانات الزائر مبكّرًا أو متأخّرًا. العمود المخصَّص هو
...
- **الهجرة إضافيّة فقط** (`ADD COLUMN archived_at`)، مكتوبة يدويًّا (لا `prisma migrate dev` — لأنّه سيحاول
  إسقاط عمود الـ FTS المُولَّد `search_vector`، `D09-6`). كُتبت إضافيّة فقط تحديدًا لأنّها
  آمنة للتطبيق على قاعدة حيّة دون توقّف.
...
- **الردّ الإداريّ مسموح بقرار `D02-13`** (وهو ينقض شقّ الردّ من `D02-4`)، **ومع تسليم فعليّ**: الصفّ يُنشأ
...
- **تطهير الرسائل بعد ١٢ شهرًا مُنفَّذ** (قسم «الاحتفاظ والتطهير» أعلاه، `D19-10`).
