...
| `idempotency-key.pipe.ts` | التحقّق من ترويسة `Idempotency-Key` كـ pipe، أي **قبل** أيّ قراءة من قاعدة البيانات. وهو **موضع الإنفاذ الوحيد لشكل المفتاح**: الخدمة تخزّنه كما جاء، والعمود `String` حرّ — والفرادة لكلّ رسالة مسنودة في `DB`، أمّا الطول والمحارف فلا. وهذا هو **الوضع الطبيعيّ لقيود المقاس** في هذا المستودع لا استثناءً فيه. أمّا **الشكل** فمختلط: شكل هذا المفتاح بلا سند، بينما شكل `slug` المهارة وشكل `hash` الوسائط يسندهما قيدا `CHECK` في `SQL` الهجرات ([التفصيل](../access-control/README.md)) |
