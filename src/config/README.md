...
`PUBLIC_WEB_URL`. ما يخصّ هذا الملفّ هو **شكل** التحقّق لا محتواه: منه المطلق (`DATABASE_URL`)، ومنه
المشروط بوضعٍ آخر (`STORAGE_LOCAL_DIR` للمحرّك المحلّي، و`S3_*` لمحرّك `s3`)، ومنه المجموعة الساكنة
حتّى تُفعَّل (`SMTP_*` خلف `SMTP_ENABLED`).
