...
الاتصال** كلّها الآن من مسؤولية `node-postgres`، لا `Prisma` (`D16-12`). هذا سلوك مرئي من خارج التطبيق،
ولا يكفي اعتباره تفصيل تنفيذ.

> **نتيجة عملية مُلزِمة:** في الإنتاج يجب أن يكون مضيف `DATABASE_URL` هو `127.0.0.1` حرفيًّا،
> لا `localhost` (`D19-12`). قاعدة `trust` في `pg_hba.conf` مقصورة على `IPv4`، و`localhost`
