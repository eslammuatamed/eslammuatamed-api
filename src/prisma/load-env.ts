/**
 * Loads `.env` for scripts that run as their OWN Node process (F9-10).
 *
 ...
 *
 * Node 24's built-in loader, not `dotenv` — P9-8 removed that dependency deliberately.
 *
 * Two properties this relies on, both deliberate:
 *   - `loadEnvFile` does NOT overwrite variables that are already set (P9-9), so a harness or a
