...
export const ARTICLES: readonly ArticleSeed[] = [
  ...
  },
  {
    ...
    ar: {
      ...
      excerpt:
        'لماذا بقي `main` مثبَّتًا عند خط أساس الإنتاج بينما استمر تدفّق العمل على الميزات إلى `dev`، والترتيب الصارم القائم على التزامات الدمج الذي اتّبعته الترقية عند رفع التجميد.',
      body: '## تجميد ما كان يعمل، لا ما كان يتغيّر\n\nتحتاج كل قاعدة شيفرة نشطة عند نقطة ما إلى حدّ فاصل بين "ما يُبنى" و"ما يعمل فعليًا في الإنتاج". في هذه الحالة كان الحدّ تجميد الإصدار: بقي فرع `main` مثبَّتًا عند خط أساس الإنتاج، بينما استمر العمل على الميزات عبر مساره المعتاد — من فرع الميزة إلى طلب سحب إلى `dev`. لم يتوقف التطوير؛ توقفت فقط ترقية ذلك التطوير إلى الإنتاج.\n\n## ترتيب صارم للترقية\n\nعندما حان وقت رفع التجميد، اتبعت الترقية تسلسلًا ثابتًا لم يجز تخطي أي خطوة منه حتى تحت ضغط الوقت: دمج `dev` في `main`، ثم توفير أو تحديث الرابط العام الذي اعتمد عليه الإصدار، ثم النشر وتشغيل فحص دخان بعد النشر. اعتمدت كل خطوة على أن سابقتها حدثت فعلًا، لا أنها كانت مقصودة فقط. التجميد الذي يمكن تجاوزه تحت الضغط ليس تجميدًا — بل مجرد اقتراح.\n\n## التزامات دمج، لا سحق\n\nدمج `dev` في `main` هو دائمًا التزام دمج (merge commit)، لا سحق (squash) أبدًا. سحق هذا الدمج تحديدًا يضغط كل تاريخ ما تم شحنه في التزام واحد، ما يجعل التقسيم الثنائي اللاحق شبه عديم الفائدة — تفقد القدرة على معرفة أي تغيير فردي داخل ترقية كبيرة تسبّب فعليًا في تراجع. السحق مناسب لفرع ميزة واحد بقصد واضح؛ لكنه الأداة الخطأ لترقية تحمل أسابيع من تغييرات مستقلة.\n\n## الترحيلات تُنشر فقط\n\nتخرج تغييرات المخطط عبر `prisma migrate deploy` ولا شيء غيره. لا `db push` عشوائي، ولا تعديلات يدوية على مخطط قاعدة بيانات حيّة. يشغّل `migrate deploy` بالضبط الترحيلات المُلتزَم بها مسبقًا في المستودع، بالترتيب، دون انزياح بين ما في ضبط الإصدار وما في قاعدة البيانات. قيد صغير، لكنه الفارق بين تاريخ ترحيل يمكن تدقيقه وآخر يجب إعادة بنائه من الذاكرة.\n\n## لماذا يستحق هذا الانضباط\n\nلا شيء من هذا غريب. التجميد، والترقية المرتّبة، والتزامات الدمج بدل السحق، وأمر ترحيل واحد — كلها خيارات بسيطة. لكن حوادث الإنتاج نادرًا ما يسبّبها خطأ درامي واحد — بل يسبّبها تخطي خطوة بسيطة مرة واحدة، تحت الضغط، لأن كل شيء بدا سليمًا. وُجد التجميد تحديدًا للحظات التي كان فيها تخطيه مغريًا.',
      readingTimeMin: 2,
    },
  },
  {
