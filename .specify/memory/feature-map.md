...
| 004 | redirects-contact-preview | M2 | SlugRedirect resolve (D10-7) + auto-on-published-rename (**D04-6**); contact intake + anti-spam + inbox; per-type HMAC preview-token mint (**D10-11**) + consume (D10-8/D19-7) | ✅ **Released through the first post-freeze API promotion** (`fb88674`, PR #54, 2026-08-06). Original implementation evidence: `c1493a1`; PRs #29/#30/#31; 364 unit + 82 e2e; contract additive; security+code reviewed. Doc-first: doc 04 v1.1.0, doc 10 v1.4.1. |
| 005 | api-hardening | M2→M5 | Retention purge (12-mo archived-message, doc 19 §6/**D19-10**) + NFR-006 latency smoke (doc 20 §5/**D20-7**) + hardening audit-pass (throttle tiers/body-limit/RFC 7807/redaction/trust-proxy — verify & regression-cover, already shipped). **Backup workflow deferred** (feature-map's "D23-5" is **superseded by D23-12**: VPS host `pg_dump` cron + weekly R2 offsite). The former release-freeze blocker ended on 2026-08-06 (D17-5/D23-18); remaining backup operations are tracked on their own merits. | 🧪 In progress on `feature/005-api-hardening` (off dev `c1493a1`); doc-first docs 09 v1.5.0 / 19 v1.4.0 / 20 v1.3.0 committed. |
...
| 008 | profile-contract | M4 | Profile pages data contract: About fields + portrait + public emails on `SiteSettings`, `ExperienceTechnology` Skill-registry join, `settings-portrait` media usage, additive public/admin shapes, deterministic seeds, and a two-layer guard protecting the raw-SQL FTS objects (D09-6) — docs 02 §3/§5, 09 §3, 10 §6, 11, 18, 22 | 🧪 Implemented on `feature/profile-contract` (PR #37); docs-first `fdf7bd6`; 397 unit + 106 e2e green; contract additive; pending owner review. |
