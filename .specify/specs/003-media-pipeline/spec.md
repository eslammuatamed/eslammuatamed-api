...
**Milestone:** M2 — API Complete (doc 24 §2). **Status:** 🚧 **In progress** — approved & underway (Q1–Q4 resolved 2026-07-18). Doc-first committed (docs branch `2623b15`, PR #2 open); **T1–T10 complete** (T8 = permission wiring + upload throttle `7c8d3e4`; T9 = Swagger + OpenAPI contract export `aaab13c`; T10 = e2e suite + CI + nullable-descriptor contract fix `d1a3811`/`07b128a`; local/unpushed, pending review), **next task T11**. API branch local/unpushed. (T7 note: page-SEO OG descriptor deferred — no public SEO endpoint exists yet; résumé exposed via `GET /settings/site` as a descriptor only.)
