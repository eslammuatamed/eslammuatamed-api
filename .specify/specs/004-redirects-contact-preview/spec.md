...
**Independent Test**: `POST /admin/articles/{id}/preview-token` (with `articles.update`) returns `{ token, url, expiresAt }` and `no-store`, where `url` is an **absolute rendered-Web link** `${PUBLIC_WEB_URL}/preview/articles/{id}?token=…` (D10-11 v1.4.1); the **API consume route** `GET /api/v1/preview/articles/{id}?token=&locale=en` returns the draft with `no-store`; the same token on the projects route or another id → `404`; after 30 min → `404`; the draft stays `404` on the public article route.

**Acceptance Scenarios**:

1. **Given** an authenticated user with `articles.update`, **When** `POST /admin/articles/{id}/preview-token`, **Then** `2xx { data: { token, url, expiresAt } }`, `Cache-Control: no-store`, `url` = the absolute rendered-Web link `${PUBLIC_WEB_URL}/preview/articles/{id}?token=…` (D10-11 v1.4.1 — the Web page the reviewer opens; it calls the consuming API route to render the draft); token value is never logged. Same for projects with `projects.update`.
...
6. **Preview mint endpoints (D10-11 — new decision).** Dedicated per-type endpoints: `POST /admin/articles/{id}/preview-token` (`@RequirePermission('articles.update')`) and `POST /admin/projects/{id}/preview-token` (`@RequirePermission('projects.update')`) → `2xx { data: { token, url, expiresAt } }`, `Cache-Control: no-store`, where **`url` is an absolute rendered-Web URL** `${PUBLIC_WEB_URL}/preview/{articles|projects}/{id}?token=<token>` (D10-11 v1.4.1 — the dashboard shares it verbatim; the API signs, the Web renders by calling the consuming `GET /api/v1/preview/…` route). `PUBLIC_WEB_URL` is a dedicated config (not `CORS_ORIGIN`; dev-default, prod-required). `token` is returned separately. Tokens are **never** embedded in admin GET/list responses (no GET side effects); token/url never logged. **Cross-repo:** the Web app must add the rendered `/preview/{articles,projects}/{id}` routes.
...
- The mint **`url`** is an **absolute rendered-Web URL** (`${PUBLIC_WEB_URL}/preview/{type}/{id}?token=`) the dashboard shares verbatim (owner decision 2026-07-20 → D10-11 v1.4.1). The API **signs**; the Web **renders** the page, which calls the consuming `GET /api/v1/preview/…` API route. `PUBLIC_WEB_URL` is a dedicated config (validated absolute http/https; dev-default `http://localhost:3000`, required in production). The preview story is therefore **cross-repo** — the Web app adds the rendered `/preview/{articles,projects}/{id}` routes (tracked as the T11 gate's Web adoption).
