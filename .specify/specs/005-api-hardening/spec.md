...
3. **NFR-006 latency smoke (doc 20 §5 +D20-7).** A `test/latency.e2e-spec.ts` that runs in the existing Postgres e2e lane (it is a `.e2e-spec.ts`, so `npm run test:e2e` — the CI e2e job — executes it; no separate ci.yml step needed): a warmup pass is discarded, then public read endpoints are sampled and p95 is asserted < a **coarse, env-overridable budget** (`LATENCY_SMOKE_P95_MS`, default 500 ms — a regression tripwire, **not** the literal 200 ms prod SLO on CI hardware), with every sampled response required 2xx. The deterministic bounded-query-count (anti-N+1) gate is **deferred** (D20-7; the seed lacks a multi-row relational public read; N+1 stays a doc 20 §7 review concern).
...
- **FR-005-04** — The API PR pipeline (Postgres e2e lane) runs a coarse p95 latency smoke over public read endpoints — warmup discarded, every sampled response 2xx — asserting p95 below a documented, env-overridable budget (a regression tripwire, not the 200 ms SLO). A deterministic bounded-query-count anti-N+1 gate is deferred (D20-7). (doc 20 §5/§7, NFR-006, D20-7)
...
- **AD-3 — Latency smoke = coarse p95 (generous, warmup-discarded), in the e2e lane; query-count deferred.** doc 20 §5 says "coarse guard … not a load test"; a literal 200 ms wall-clock assertion is flaky on shared CI runners, so the budget is a generous env-overridable tripwire (default 500 ms) with a discarded warmup pass, and every sampled public read must be 2xx. A deterministic per-request **query-count** (anti-N+1) gate — the advisor-recommended non-flaky signal — was **deferred**: the current seed has no multi-row relational public read to make it meaningful, and instrumenting the app's `PrismaService` for query events would be a production change for a test. N+1 stays prevented structurally + at review (doc 20 §7); the coarse p95 also trips on a gross blow-up. Recorded in D20-7 as a future enhancement. **Alternatives rejected:** literal 200 ms CI assertion (flaky); a real load test (doc 18 §7 "monitored, not load-simulated"); throttle-config relaxation for the smoke (unneeded — bounded samples stay under 120/min in a fresh app instance).
...
## Review outcomes (Phase 5 — separate lane: security + code/contract/test)

- **Security review: sound, no Critical/High.** Confirmed the purge deletes only genuinely-old archived rows (never unarchived/recent), the body-limit change drops no security middleware and has no bypass, and the exception-filter change leaks no internals and never downgrades a 5xx. No control weakened; migration + contract strictly additive; no secret/.env exposure.
- **Code review: no blockers; 2 Mediums fixed.**
  - **M1 (fixed)** — the new client-http-error filter branch's 400/malformed-JSON handling was dead in prod (Nest's `mapExternalException` wraps a JSON `SyntaxError` into a `BadRequestException` *before* the filter), and a unit test fabricated a shape prod never emits. Fixed: removed the dead `BAD_REQUEST` detail arm + the misleading unit test, corrected the comments to state the **live** case is the 413 over-limit body (`PayloadTooLargeError`, which reaches the branch unwrapped and renders correctly). Added an e2e proving malformed JSON → 400 `application/problem+json`.
  - **M2 (fixed)** — the >1 MiB e2e asserted only `status===413`; strengthened to assert `application/problem+json` + `type=/problems/payload-too-large` (locks the RFC 7807 rendering, not a bare Express 413), tightened the under-1 MiB assertion to `[200,422]`, and added the malformed-JSON e2e.
  - **Nits (fixed):** N1 latency-comment arithmetic; N2 `retentionCutoff` whole-year/leap-day note.
- **Accepted & documented (non-blocking, per both reviewers):**
  - **LOW-1 — `/contact` honeypot `website` field has no `@MaxLength`, so AD-7's 1 MiB transport cap lets a throttled unauthenticated client send up to ~1 MiB there.** Accepted: the field is **never persisted or logged** (read by `isSpam()`, then discarded; the persisted fields stay capped, `body` ≤ 5000), and `POST /contact` is throttled 3/hr + 10/day per IP, so impact is only transient parse-memory on a throttled endpoint. **Not fixed by adding `@MaxLength`** — a distinguishable 422 would leak the honeypot (D02-1). Mitigation if ever wanted: a per-route body cap on `POST /contact` (deferred as unnecessary at personal-site scale).
  - **LOW/INFO-2 — a persistently-failing purge is `console.error`'d by `@nestjs/schedule` (bypassing structured pino), retried next daily tick.** Matches the shipped `ArticlesScheduler` (D07-3) pattern; `@nestjs/schedule` prevents any crash; FR-005-03's "logged" is satisfied. Kept consistent with the established pattern rather than diverging for one job.
  - **INFO-3** — pre-migration archived rows (`archivedAt = NULL`, no backfill) are never purged (safe direction — errs toward retention; moot pre-launch).
  - **INFO-4** — an uncommon body-parser 4xx (e.g. 415) surfaces with the correct status but a generic "Internal Server Error" title (cosmetic RFC 7807 label; no leak; the live 413 is labeled correctly).
  - **L1** — `main.ts` uses `bodyParser:false` + explicit `express.json/urlencoded` (a documented NestJS approach, fully tested). `app.useBodyParser('json',{limit})` is a cleaner Nest-native alternative; kept the tested approach (reviewer: "defensible to keep").
  - **L3** — the retention e2e invokes the real table-wide purge (self-cleaning; safe today — no other suite seeds >12-month archived rows).
