...
> **المسار الخاص للنسخ الخارجي تشغيلي ومُفعَّل.** تبقى بيانات قاعدة البيانات في bucket R2 خاص،
> واعتماد `rclone` محصورًا بالخادم، بينما يسمح `BACKUP_OFFSITE_ENABLED=true` بالتشغيل الأسبوعي
> وفق `17 6 * * 0` (الأحد 06:17 UTC). لا يمنح تفعيل النسخ أي تفويض لنشر تطبيق Production.
...
متغير GitHub غير السرّي `BACKUP_OFFSITE_ENABLED` هو بوابة التشغيل والإيقاف الصريحة: القيمة `true`
تسمح بالتشغيل المجدول أو اليدوي، بينما القيم الغائبة أو الفارغة أو `false` تمنع job
`trigger-vps-backup` التشغيلي من البدء أو إنشاء أي مادة SSH. لذلك يبقى المتغير وسيلة الإيقاف
الطوارئ والتشغيل المقصودة لهذه العملية.
