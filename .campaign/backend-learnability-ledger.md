...
| Fact | Value |
| --- | --- |
| Campaign branch | **MERGED.** `campaign/backend-learnability` reached `dev` through PR #86 (`e87f427`, 2026-08-18). The campaign has no live integration branch of its own anymore — later changes land on `dev` like any other work |
| Campaign baseline (historical) | `9af1aac` — the tree the campaign started from, back when `origin/dev` and `origin/main` coincided with it. Both branches have moved on since; live branch state is read from git, never from a SHA pair stamped here |
| Commits on branch | closed history: `git log 9af1aac..e87f427` on the merged branch — do not trust a count typed here |
...
| Peer lane (runs 7–8) | **CLOSED — `CLEAN` at depth.** Ten rounds: 20 · 7 · 4 · 7 · 3 · 3 · 1 · CLEAN · *CLEAN(shallow, discounted)* · **CLEAN (8,976 lines / 43 tool calls, traces shown)**. The last two were narrow and claim-numbered; the lane ended because a round found nothing, not on a count. The final `CLEAN` was taken on `55201e4` and is valid for that tree alone — an earlier version of this row predicted "every later commit touches `.campaign/` only", and later `dev` work (contract and deploy fixes) broke that prediction, which is exactly why tree-scoped readings are never stamped as permanent |
...
| Campaign exit gate | Run 9 was **NOT executed**. The drafted prompt lived only in an uncommitted scratchpad and never became a durable repository artifact; the campaign PR has since merged, so the prompt/bundle pairing no longer exists anywhere checkable. Nothing is owed by the campaign. The owner may commission a fresh external cold-reader exercise separately if ever wanted — and neither `Claude` nor `Codex` may be the reader |
| Contract gap: `POST /admin/media` undeclared `400` | **RESOLVED by PR #98** (`de5bfe5`): `openapi.json` now declares `400`, `413`, and `429` with `Retry-After` for `POST /api/v1/admin/media`, pinned by the contract spec under `src/contract/`. The authoritative status matrix is OpenAPI itself plus its tests — not this table |
| Media upload compensation limits | **RESOLVED by PR #90** (`0355e46`): cleanup failure is logged (`media.compensation_incomplete`) and no longer replaces the original error; compensation is bounded at confirmed `mediaAsset.create`; post-persistence mapping/logging failures do not delete stored objects. Best-effort cleanup may still orphan storage objects, and DB commit/network ambiguity remains an accepted external-system limitation — both are stated in the source comments, not tracked here |
| OD-A (Prisma 7 governance record) | **RESOLVED — owner-confirmed docs PR #56.** Governing docs now record the adopted Prisma 7 policy as `D16-12`; `D16-13` remains absent and was not invented or backfilled. §8 remains historical analysis and is intentionally unchanged |
...
| Rollback policy when no verified healthy target exists | **RESOLVED 2026-08-23 — owner decision: keep the current behavior.** If no previous release exists, the failed new release remains `current`; if rollback selects `PREV` but that target also fails the same acceptance verification, that previous release remains `current`. In both cases deployment fails with `MANUAL INTERVENTION REQUIRED` and the condition is treated as an ongoing outage. No service-stop / fail-closed behavior is requested; `CONTRIBUTING.md`, `scripts/deploy/README.md`, and `remote-cutover.sh` already define the behavior. |
